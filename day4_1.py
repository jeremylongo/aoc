...
best_candidate = 0
score = 0
for k,v in total_per_guard.items():
    if v > score:
        best_candidate = k
        score = v

print ("%s: %s" % (best_candidate, score))

total_minutes = {}
best_minute = 0
best_minute_count = 0
for d, minutes in guard_status[best_candidate].items():
    for m in minutes.keys():
        if m not in total_minutes:
            total_minutes[m] = 1
        else:
            total_minutes[m] += 1
        if total_minutes[m] > best_minute_count:
            best_minute_count = total_minutes[m]
            best_minute = m

for m, c in total_minutes.items():
    print("%s : %s" % (m, c))

print("result : guardian %s, minute %s, id=%s" % (best_candidate, best_minute, best_candidate * best_minute))
